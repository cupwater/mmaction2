...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc_cls = nn.Linear(self.feat_dim, num_classes)
    ...
    def forward(self, x):
        ...
        for i, layer_name in enumerate(self.res_layers):
            ...
            x = res_layer(x)
        x = self.avgpool(x)
        out = self.fc_cls(x.reshape(x.shape[0], -1))
        return out
...
if __name__ == "__main__":
    ...
    path = "work_dirs/tsn_r18_1x1x8_100e_lege_rgbdiff/latest.pth"
    weights = torch.load(path, map_location='cpu')['state_dict']
    new_weights = {}
    for k, v in weights.items():
        new_weights[k.replace('backbone.', '').replace('cls_head.', '')] = v
    net.eval()
    net.load_state_dict(new_weights)
    torch.save(new_weights, 'test.pth')


    model_path = 'test.onnx'
    dummy_input = torch.randn(1, 3, 224, 224) #.to("cuda")
    # dummy_input = torch.randn(1, 3, 480, 640).to("cuda") #if input size is 640*480
    torch.onnx.export(net, dummy_input, model_path, verbose=False, opset_version=11, input_names=['input'], output_names=['scores'])
    from onnxsim import simplify
    import onnx
    onnx_model = onnx.load(model_path)
    model_simp, check = simplify(onnx_model)
    onnx.save(model_simp, 'tsn_r18_1x1x8_100e_lege_rgbdiff.onnx')
